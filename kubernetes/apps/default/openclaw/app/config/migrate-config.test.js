...
test("migrates a fresh config to three includes with byte idempotence", () => {
...
test("migrates legacy config, removes Gemini, and preserves unrelated fields", () => {
  const input = {
...
test("normalizes existing includes without retaining sibling keys", () => {
  ...
  for (const section of ["gateway", "models", "tools"]) {
...
test("managed fragments are Git-owned and contain no credential literals or Gemini", () => {
  ...
  const names = ["gateway.json5", "models.json5", "tools.json5"];
  const contents = names.map((name) =>
    fs.readFileSync(path.join(managedDirectory, name), "utf8"),
  );
  const combined = contents.join("\n");

  for (const content of contents) assert.match(content, /Git-owned and read-only/);
