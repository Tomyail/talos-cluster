...
After bootstrap, the remaining flow is:

1. **Bitwarden Setup**: OAuth credentials configured in the SOPS-encrypted `bitwarden-cli` secret
2. **ClusterSecretStore Creation**: the Bitwarden ESO provider creates `bitwarden-login` and `bitwarden-fields` stores
3. **ExternalSecret Reconciliation**: External Secrets Operator reads ExternalSecret CRs and syncs secrets from Bitwarden

## Secret Synchronization Workflow

### Bootstrap Flow

During initial cluster bring-up, `scripts/bootstrap-apps.sh` installs the SOPS-managed secrets **before** Flux reconciles any charts (`scripts/bootstrap-apps.sh#L57-L85`):

1. `apply_namespaces` creates one namespace per top-level directory under `kubernetes/apps/` (server-side apply).
2. `apply_sops_secrets` decrypts and applies three secrets into the `flux-system` namespace using `sops exec-file ... kubectl --namespace flux-system apply --server-side`:
   - `bootstrap/github-deploy-key.sops.yaml` (deploy key for private Flux sources)
   - `kubernetes/components/common/sops/cluster-secrets.sops.yaml`
   - `kubernetes/components/common/sops/sops-age.sops.yaml`
3. `apply_crds` installs CRDs needed by helmfile-installed core components before apps are deployed.

The age private key is **not** generated by the bootstrap script; the encrypted `sops-age` secret must already exist in Git (re-encrypt it locally with the age.key if it is new). This chicken-and-egg ordering is why `sops-age` and `cluster-secrets` are applied out-of-band into `flux-system` before the Flux `cluster-apps` Kustomization exists.

### Root Kustomization Decryption

At the Flux root, both `cluster-meta` and `cluster-apps` Kustomizations configure SOPS decryption with the `sops-age` secret (`kubernetes/flux/cluster/ks.yaml#L9-L12`, `#L74-L77`). `cluster-apps` depends on `cluster-meta`, `gateway-api-crds`, and `external-dns-crds`, so app-level SOPS files decrypt only after the cluster is bootstrapped. Per-app Kustomizations such as `bitwarden-connect` repeat the same `decryption.provider: sops` / `secretRef: sops-age` block (`kubernetes/apps/external-secrets/bitwarden-connect/ks.yaml#L15-L18`) plus `postBuild.substituteFrom: cluster-secrets` (`#L19-L22`).
